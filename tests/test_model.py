"""Exercise ModelPIIParser across diverse PII scenarios."""
...
def test_case(parser, name, text, expected_labels):
    """Check that each expected label appears at least once in predictions."""
    ...
    print()
    return {"name": name, "status": status, "dt": dt, "missing": missing, "extra": extra}
...
def main():
    ...
    # Force model warm-up with a tiny input so later timings reflect steady state.
    _ = parser.parse("hi")
    ...
    cases = [
        ...
        (
            "no_pii",
            "The weather is nice today and the clouds look fluffy.",
            [],
        ),
        (
            "dense_paragraph",
